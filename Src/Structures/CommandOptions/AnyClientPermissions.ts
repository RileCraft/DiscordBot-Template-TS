import { DiscordClient, EmbedBuilder, Interaction, Message, PermissionsBitField } from "discord.js";
...
export const anyClientPermissionsFN = (client: DiscordClient, message: Message | Interaction<"cached">, command: AnyCommand): boolean => {
    ...
    if (command.anyClientPermissions.some((permission: PermissionsBitField) => message.guild?.members.me?.permissions.has(permission))) return true;
    else {
